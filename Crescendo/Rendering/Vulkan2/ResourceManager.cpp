...
CS_NAMESPACE_BEGIN::Vulkan
{
	...
	MeshHandle TemplateUploadMesh(const cs_std::graphics::mesh<index_type>& mesh, Device* device, const Vk::TransferCommandQueue& transferQueue, cs_std::slotmap<Mesh>& meshes)
	{
		...
		for (const auto& attribute : mesh.attributes) CS_ASSERT_WARNING(attribute.data.size() % ELEMENTS_PER_ATTRIBUTE[static_cast<size_t>(attribute.attribute)] == 0, "Invalid mesh data. Mesh has " + std::to_string(attribute.data.size()) + " elements which is not a multiple of " + std::to_string(ELEMENTS_PER_ATTRIBUTE[static_cast<size_t>(attribute.attribute)]));
		...
		gpuMesh.indexBuffer = device->CreateBuffer(
			gpuMesh.indexCount * sizeof(index_type), VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT , VMA_MEMORY_USAGE_CPU_TO_GPU
		);
		for (const auto& attribute : mesh.attributes)
		{
			gpuMesh.vertexAttributes.emplace_back(
				device->CreateBuffer(attribute.data.size() * sizeof(float), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU),
		...
		Vk::Buffer indexStaging = device->CreateBuffer(mesh.indices.size() * sizeof(index_type), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
		indexStaging.memcpy(mesh.indices.data(), mesh.indices.size() * sizeof(index_type), 0);

		Vk::Buffer staging = device->CreateBuffer(bufferOffsets.back(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
	...
	ResourceManager::ResourceManager(Device& device) : device(&device), transferQueue(device, device.GetTransferQueue(), false) {}
	ResourceManager::ResourceManager(ResourceManager&& other) noexcept : device(other.device), transferQueue(std::move(transferQueue)), buffers(std::move(other.buffers)), textures(std::move(other.textures)), meshes(std::move(other.meshes)) {}
	...
	BufferHandle ResourceManager::CreateBuffer(VkDeviceSize size, VkShaderStageFlags shaderStage)
	{
		...
		return BufferHandle(buffers.emplace(
			this->device->CreateBuffer(size, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU),
			BindlessDescriptorManager::BufferHandle::Invalid
		));
	}
	TextureHandle ResourceManager::UploadTexture(const cs_std::image& image, const TextureSpecification& textureSpec)
	{
		...
		sampler = this->samplers.back();

		/* ----------------------------------------------------------------  2. - Staging ---------------------------------------------------------------- */
		Vk::Buffer staging = this->device->CreateBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
		staging.memcpy(image.data.data(), imageSize, 0);
		const VkExtent3D extent = Create::Extent3D(image.width, image.height, 1);
		Vk::Image texture = this->device->CreateImage(Vulkan::Create::ImageCreateInfo(
		...
		/* ----------------------------------------------------------------  4. - Handle Creation ---------------------------------------------------------------- */
		return TextureHandle(textures.emplace(
			std::move(texture),
			sampler,
			BindlessDescriptorManager::ImageHandle::Invalid
		));
	}
